...
| `data-outbound-links` | 否 | 设置为 `true` 自动追踪站外链接点击 |
| `data-file-downloads` | 否 | 设置为 `true` 自动追踪文件下载（pdf、zip 等） |
| `data-file-types` | 否 | 自定义下载追踪的文件扩展名列表，逗号分隔（默认：`pdf,xlsx,docx,txt,rtf,csv,...`） |
...
```html
<!-- 追踪按钮点击 -->
<button class="event-name=注册+点击">注册</button>

<!-- 追踪链接点击 -->
<a href="/pricing" class="event-name=定价+点击">查看定价</a>

<!-- 追踪表单提交 -->
<form class="event-name=联系表单+提交">
  <input type="email" />
...
这会发送事件名 `Purchase`，属性 `{ plan: "Pro", amount: "99" }`。

**说明：**

- 使用 `+` 表示事件名称中的空格（例如，`按钮+点击` 变为 "按钮 点击"）
- 类名可以放在元素或任何父元素上（最多向上搜索 3 层）
- 对于链接，`url` 属性会自动设置为链接的 `href`
...
在 script 标签上添加 `data-outbound-links="true"` 启用自动追踪：

```html
<script defer data-domain="yourdomain.com" data-outbound-links="true" src="..."></script>
```

启用后，指向外部域名的链接点击会被追踪为 `Outbound Link: Click` 事件，附带 `url` 属性。

### 文件下载追踪

在 script 标签上添加 `data-file-downloads="true"` 启用文件下载追踪：

```html
<script defer data-domain="yourdomain.com" data-file-downloads="true" src="..."></script>
```

这会追踪常见文件格式（pdf、zip、docx 等）的点击。自定义文件类型：
...
表单提交会自动追踪为 `Form: Submission` 事件。如果表单带有 `event-name=*` 类名，则会使用该自定义事件名提交。
