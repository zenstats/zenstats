...
ZenStats 提供与 [Plausible Analytics](https://plausible.io/docs/stats-api) 兼容的统计查询 API。所有 API 均需要 Bearer Token 认证。
...
获取聚合统计数据，对标 Plausible `GET /api/v1/stats/aggregate`。
...
获取时序图表数据，对标 Plausible `GET /api/v1/stats/timeseries`。
...
按维度获取排行数据，对标 Plausible `GET /api/v1/stats/breakdown`。
...
获取当前在线访客数，对标 Plausible `GET /api/v1/stats/realtime/visitors`。
...
| 功能           | ZenStats 接口                                    | Plausible 对应接口                    |
|---------------|--------------------------------------------------|--------------------------------------|
| 总览指标        | `GET /stats/:d/aggregate`                         | `GET /api/v1/stats/aggregate`        |
| 主图表          | `GET /stats/:d/main-graph`                        | `GET /api/v1/stats/timeseries`       |
| 维度细分        | `GET /stats/:d/breakdown`                         | `GET /api/v1/stats/breakdown`        |
| 实时访客        | `GET /stats/:d/current-visitors`                  | `GET /api/v1/stats/realtime/visitors`|
| 来源排行        | `GET /stats/:d/source_rank` / `GET /stats/:d/sources` | `GET /api/v1/stats/breakdown?property=visit:source` |
| 设备排行        | `GET /stats/:d/device_rank`                       | `GET /api/v1/stats/breakdown?property=visit:device` |
| 页面排行        | `GET /stats/:d/page_rank`                         | `GET /api/v1/stats/breakdown?property=event:page` |
| 顶部概览        | `GET /stats/:d/top_stats`                         | `GET /api/v1/stats/aggregate` (组合)  |
| 曲线图          | `GET /stats/:d/curve`                             | `GET /api/v1/stats/timeseries`       |
