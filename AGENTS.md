...
自托管网站分析服务。Go 1.24+ / Gin v1.10 后端，PostgreSQL（业务数据，ent ORM）+ ClickHouse（事件/Session 分析数据）。前端管理面板由独立仓库 zenstats-web 维护，部署编排在 zenstats-deploy。
...
entrypoint.sh            # 容器入口脚本 (自动 migrate + 启动)
```

> 前端管理面板（React SPA）和埋点 SDK（Tracker JS）由独立仓库 [zenstats-web](https://github.com/zenstats/zenstats-web) 维护。
...
| 用途 | 命令 |
|------|------|
| 启动开发 DB | `cd ../zenstats-deploy && docker compose -f docker-compose.test.yml up -d` |
