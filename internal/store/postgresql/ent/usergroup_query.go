...
type UserGroupQuery struct {
	...
	withUserConfigs *UserConfigQuery
...
func (ugq *UserGroupQuery) QueryUserConfigs() *UserConfigQuery {
	query := (&UserConfigClient{config: ugq.config}).Query()
...
func (ugq *UserGroupQuery) Clone() *UserGroupQuery {
	...
	return &UserGroupQuery{
		...
		withUserConfigs: ugq.withUserConfigs.Clone(),
...
func (ugq *UserGroupQuery) WithUserConfigs(opts ...func(*UserConfigQuery)) *UserGroupQuery {
	query := (&UserConfigClient{config: ugq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ugq.withUserConfigs = query
	return ugq
}
...
func (ugq *UserGroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserGroup, error) {
	...
	if query := ugq.withUserConfigs; query != nil {
		if err := ugq.loadUserConfigs(ctx, query, nodes,
			func(n *UserGroup) { n.Edges.UserConfigs = []*UserConfig{} },
			func(n *UserGroup, e *UserConfig) { n.Edges.UserConfigs = append(n.Edges.UserConfigs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ugq *UserGroupQuery) loadUserConfigs(ctx context.Context, query *UserConfigQuery, nodes []*UserGroup, init func(*UserGroup), assign func(*UserGroup, *UserConfig)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*UserGroup)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userconfig.FieldGroupID)
	}
	query.Where(predicate.UserConfig(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usergroup.UserConfigsColumn), fks...))
	}))
